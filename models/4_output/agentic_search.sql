...
STAK AS (
    ...
    SELECT date
        ...
        , {{ default_null() }} AS brand_present
        , 0 AS brand_response_rank
        , occurrences
        , 0 AS negative
        , 0 AS positive
        , 0 AS positive_sentiment
...
SELECT STAK.date
    ...
    , CASE WHEN {{ qualify ('demo.agentic_competitors','(STAK.asset_name)')}} = 'unknown' THEN NULL
        ELSE {{ qualify ('demo.agentic_competitors','(STAK.asset_name)')}} END AS brand_competitor
    , STAK.model
